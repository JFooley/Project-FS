...
public class WGAccessibilityMenu : Widget {
    ...
    private static Selector selector = new Selector(new List<int> {1, 1, 1, 1, 1});
    ...
    public override void Render() {
        ...
        // 2
        if (UI.DrawButton(Language.GetText("high contrast") + ": " + Language.GetText(Accessibility.high_contrast ? "enabled" : "disabled"), anchor.X, anchor.Y+2*n, hover: selector.is_on(0, 2), click: Input.Key_hold("A"), action: Input.Key_up("A"), spacing: Config.spacing_small, click_font: "default small click", hover_font: "default small hover", font: "default small", alignment: "Left"))
            Accessibility.high_contrast = !Accessibility.high_contrast;

        // 3
        if (UI.DrawButton(Language.GetText("atack feedback") + ": " + Language.GetText(Accessibility.atack_feedback ? "enabled" : "disabled"), anchor.X, anchor.Y+3*n, hover: selector.is_on(0, 3), click: Input.Key_hold("A"), action: Input.Key_up("A"), spacing: Config.spacing_small, click_font: "default small click", hover_font: "default small hover", font: "default small", alignment: "Left"))
            Accessibility.atack_feedback = !Accessibility.atack_feedback;

        // 4
        if (UI.DrawButton(Language.GetText("save and exit"), anchor.X, anchor.Y+5*n, hover: selector.is_on(0, 4), click: Input.Key_hold("A"), action: Input.Key_up("A"), spacing: Config.spacing_small, click_font: "default small click", hover_font: "default small hover", font: "default small", alignment: "Left")) {
            Config.SaveToFile();
            Camera.LockCamera();
        ...
        if (Input.Key_down("B")) selector.pointer = new Vector2i(0, 4);
